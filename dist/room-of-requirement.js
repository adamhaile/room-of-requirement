...
let proxy = (_, base, instances) => new Proxy(overlay(_, base), {
    ...
    for (var node = resolve(_, target); node !== undefined; node = node.cache.parent ? resolve(node.cache.parent, target) : undefined)
        nodes.push(node);
    let value = nodes.map(node => node instanceof Instance ? node.value : proxy(node.cache, target, null)), cache = nodes.length ? nodes[0].cache : _;
    ...
    if (obj instanceof Object && Object.getPrototypeOf(obj) === Object.prototype) {
        ...
        if (_.items[path] instanceof NestedRules)
...
exports.root = proxy(new CacheState(0, Object.create(null), null), '', null);
